...
import static iso20022.Iso20022ModelConfig.*;
...
public class BANKABCTestPackCreator {
    ...
    void clean() throws IOException {
        Path path = TestingExpectationUtil.TEST_WRITE_BASE_PATH.get();
        if (null != path) {
            // These will all get regenerated by the PipelineConfigWriter
            ...
            deleteDirectory(path.resolve("regulatory-reporting/input/cftc-event-scenarios").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/commodity").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/credit").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/custom-scenarios").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/delegated-reporting").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/equity").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/etd").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/events").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/fx").toFile());
            deleteDirectory(path.resolve("regulatory-reporting/input/rates").toFile());
            deleteDirectory(path.resolve("projection").toFile());
        }
    }

    private PipelineTreeConfig createReportableEventTreeConfig(String prefix) {
        return getWithOutputSerialisationFormatMap(new PipelineTreeConfig(prefix)
    ...
    private static PipelineTreeConfig getWithOutputSerialisationFormatMap(PipelineTreeConfig pipelineTreeConfig) {
        return pipelineTreeConfig.withXmlConfigMap(TYPE_TO_XML_CONFIG_MAP)
                .withXmlSchemaMap(TYPE_TO_SCHEMA_MAP)
                .withOutputSerialisationFormatMap(TYPE_TO_FORMAT_MAP);
    }
